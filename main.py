...
if __name__ == "__main__":
    ...
    # Get the VaR
    percentile = 0.01 # CI
    var = sp.value_at_risk_hist(percentile = percentile)
    ...
    var_cov = sp.value_at_risk_var_covar(percentile = percentile)
    print('\nAccording to the variance-covariance method, the VaR for a ' + str((1 - percentile) * 100)+ '% CI' +
          ' is ' + str(abs(np.round(var_cov * 100,2))) + '%.')

    var_monte_carlo = sp.value_at_risk_monte_carlo(repetitions = 1000, percentile=percentile)
